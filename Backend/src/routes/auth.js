...
router.post("/login", async (req, res) => {
  const { email, password, admissionNumber, firstName, passcode } = req.body;
  ...
  if (admissionNumber) {
    ...
    if (!student)
      return res.status(401).json({ error: "Admission number not found" });

    if (student.id.slice(-6).toUpperCase() !== passcode.trim().toUpperCase()) {
    ...
    if (storedFirst !== firstName.trim().toLowerCase()) {
      return res
        .status(401)
        .json({ error: "First name does not match our records" });
    }
  ...
  await log({
    ...
    entity: "User",
    entityId: user.id,
    detail: `${user.role} "${user.name}" logged in`,
  });
