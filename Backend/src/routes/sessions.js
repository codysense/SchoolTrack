...
router.post('/', adminOnly, async (req, res) => {
  ...
  if (!name?.trim()) return res.status(400).json({ error: 'Session name is required (e.g. 2024/2025)' })
  ...
  try {
    ...
    await log({ req, action: 'CREATE', entity: 'Session', entityId: session.id, detail: `Created session ${session.name}` })
    res.status(201).json(session)
  } catch (e) {
    if (e.code === 'P2002') return res.status(409).json({ error: 'A session with that name already exists' })
    throw e
  }
})
...
router.put('/terms/:termId/set-current', adminOnly, async (req, res) => {
  ...
  await prisma.session.updateMany({ where: { isCurrent: true }, data: { isCurrent: false } })

  // Set new current
  await prisma.term.update({ where: { id: termId }, data: { isCurrent: true } })
  await prisma.session.update({ where: { id: term.sessionId }, data: { isCurrent: true } })

  await log({ req, action: 'UPDATE', entity: 'Term', entityId: termId,
...
router.delete('/:id', adminOnly, async (req, res) => {
  ...
  await log({ req, action: 'DELETE', entity: 'Session', entityId: req.params.id, detail: `Deleted session ${session.name}` })
...
router.get('/terms/:termId/summary', async (req, res) => {
  ...
  const schoolFeeExpected  = students.reduce((s, st) => s + st.class.feeAmount, 0)
  ...
  for (const a of optAssigns) {
    ...
    if (!optFeeMap[id]) {
      optFeeMap[id] = { id, name: a.optionalFee.name, expected: 0, collected: 0, assignCount: 0 }
    }
  ...
  res.json({
    ...
    totalOutstanding: (schoolFeeExpected + optFeeExpected) - (schoolFeeCollected + optFeeCollected),
  })
})
