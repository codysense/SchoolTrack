...
export default function AuditLog() {
  ...
  const load = (f = filters) => {
    ...
    Promise.all([api(`/audit?${params}`), api("/audit/actors")])
      .then(([data, a]) => {
        setLogs(data.logs);
        setTotal(data.total);
        setPages(data.pages);
  ...
  const roleColor = (r) =>
    ({ ADMIN: "blue", TEACHER: "green", STUDENT: "yellow" })[r] || "gray";
    ...
    <div>
      ...
      >
        ...
        >
          ...
          {actors.map((a) => (
            <option key={a.userId || a.userName} value={a.userId || ""}>
        ...
        >
          ...
          {["CREATE", "UPDATE", "DELETE", "LOGIN", "REMIND"].map((a) => (
            ...
            >
              <thead>
                <tr style={{ background: "#f9fafb" }}>
                  {["When", "User", "Role", "Action", "Entity", "Detail"].map(
                    (h) => (
                      <th
                        key={h}
                        style={{
