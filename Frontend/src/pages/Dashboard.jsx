...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const url = selectedTermId
      ? `/payments/summary?termId=${selectedTermId}`
      : "/payments/summary";
    api(url)
      .then(setSummary)
        ...
        summary && (
          <>
            ...
            {!currentTerm && (
              ...
              >
                ⚠ No active term set. Go to <strong>Sessions</strong> and click
                "Set Active" on a term before recording payments or results.
              </div>
            )}
            ...
            >
              <StatCard
                label="Expected"
                value={fmt(summary.schoolFeeExpected)}
                color="#6366f1"
              />
              <StatCard
                label="Collected"
                value={fmt(summary.schoolFeeCollected)}
                color="#10b981"
              />
              <StatCard
                label="Outstanding"
                value={fmt(summary.schoolFeeOutstanding)}
                color="#f59e0b"
                sub={
                  summary.schoolFeeOutstanding > 0
                    ? "Needs follow-up"
                    : "✓ All clear"
                }
              />
                ...
                >
                  {summary.optFeeSummary.map((fee) => (
                    ...
                    >
                      ...
                      >
                        {fee.assignCount} student
                        {fee.assignCount !== 1 ? "s" : ""} assigned
                      </p>
                      <div
                        style={{
                          display: "grid",
                      ...
                      <div style={{ marginTop: 10 }}>
                        ...
                        >
                          <div
                            style={{
                              width:
                                fee.expected > 0
                                  ? `${Math.min(100, (fee.collected / fee.expected) * 100)}%`
                                  : "0%",
                              height: "100%",
                              background:
                                fee.outstanding <= 0 ? "#10b981" : "#f59e0b",
                              borderRadius: 4,
                            }}
                          />
                        </div>
                        <p
                          style={{
                            margin: "4px 0 0",
                            fontSize: 11,
                            color: fee.outstanding > 0 ? "#f59e0b" : "#10b981",
                            fontWeight: 600,
                          }}
                        >
                          {fee.outstanding > 0
                            ? `${fmt(fee.outstanding)} outstanding`
                            : "✓ Fully collected"}
                        </p>
                      </div>
                    </div>
              ...
              >
                <thead>
                  <tr style={{ background: "#f9fafb" }}>
                    {["Student", "Class", "Amount", "Method", "Date"].map(
                      (h) => (
                        <th
                          key={h}
                          style={{
